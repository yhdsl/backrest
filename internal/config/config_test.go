...
func TestConfig(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:   "default config",
			config: NewDefaultConfig(),
			store:  &ConfigManager{Store: &JsonFileStore{Path: dir + "/default-config.json"}},
		},
		{
			...
			store: &ConfigManager{Store: &JsonFileStore{Path: dir + "/valid-config.json"}},
		},
		{
			...
			store:           &ConfigManager{Store: &JsonFileStore{Path: dir + "/invalid-config.json"}},
			wantErr:         true,
		...
		},
		{
			...
			store:           &ConfigManager{Store: &JsonFileStore{Path: dir + "/invalid-config2.json"}},
			wantErr:         true,
		...
		},
		{
			...
			store:           &ConfigManager{Store: &JsonFileStore{Path: dir + "/invalid-config3.json"}},
			wantErr:         true,
